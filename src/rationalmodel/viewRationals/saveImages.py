...
def make_objects(spacetime, number, dim, accumulate, config, ccolor, view_objects, view_next_number, max_time, ptime):
    if not spacetime:
        return
    if number == 0:
        return
...
def _create_image(args):
    ...
    view_objects, view_next_number, max_time, image_resx, image_resy, \
    path, rotate, dx = args
    ...
    objs = make_objects(spacetime, number, dim, accumulate, config, ccolor, view_objects, view_next_number, max_time, ptime)
    if not objs:
        print('------ NOT OBJS')
        return
...
def _saveImages(args):
    ...
    for frame in range(range_frames):
        ...
        params.append((
            ...
            frame, factor, init_time, prefix, suffix, config, ccolor, spacetime, dim, number, 
